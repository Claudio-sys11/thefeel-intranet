...
회원가입·직원관리 등 **데이터는 서버 PC 한 곳에 모입니다.** 직원 PC는 서버에 접속만 합니다.

- **서버 PC(관리자)**: 앱을 실행하고 최초 1회 **"이 PC를 서버로 사용"** 선택.
  - `서버PC_방화벽허용.bat` 을 **관리자 권한으로 1회 실행**(5000 포트 인바운드 허용).
  - 설정창에 표시되는 **이 PC의 IP(예: 192.168.0.74)** 를 직원에게 알려줍니다.
  - 서버 PC는 켜져 있어야 직원들이 접속할 수 있습니다.
- **직원 PC**: 앱을 실행하고 최초 1회 **"서버에 접속"** 에 **서버 PC의 IP** 입력.
  - 이후 회원가입하면 **서버(관리자) DB로 접수**되어 관리자 직원관리에 보입니다.

> 설정을 바꾸려면 `%LOCALAPPDATA%\ThefeelIntranet\server.cfg` 파일을 삭제 후 앱을 다시 실행하세요.
> (기존 관리자 PC는 데이터가 있어 자동으로 서버 모드로 동작합니다.)
...
1. 릴리스의 **`TheFeelKorea-Setup-Tools.zip`** 다운로드 후 압축 해제
